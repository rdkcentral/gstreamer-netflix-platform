...
namespace rdk_gstreamer_utils
{
    ...
    void configAudioCap_soc(AudioAttributes *pAttrib, bool *audioaac, bool svpenabled, GstCaps **appsrcCaps)
    {
        ...
        {
            LOG_RGU("####### Using LPCM\n");
            caps_string = g_strdup_printf("audio/x-raw,format=S16LE,rate=(int)%u,channels=(int)%u,enable-svp=(string)%s",
                                        pAttrib->mSamplesPerSecond,
                                        pAttrib->mNumberOfChannels,
    ...
    void deepElementAdded_soc (struct rdkGstreamerUtilsPlaybackGrp *pgstUtilsPlaybackGroup,GstBin* pipeline, GstBin* bin, GstElement* element)
    {
        ...
        if (elementName) {
            ...
            if (g_strrstr(elementName, "brcmaudiodecoder") && isPtsOffsetAdjustmentSupported_soc()) {
            ...
            if (g_strrstr(elementName, "brcmaudfilter")) {
                GstElement* audfilterParent = (GstElement *) gst_element_get_parent(element);
                if (audfilterParent) {
                    gchar* audfilterParentName = gst_element_get_name(audfilterParent);
                    if (audfilterParentName && g_strrstr(audfilterParentName, "bin")) {
                        LOG_RGU( "setting use-buffering FALSE\n" );
                        g_object_set( G_OBJECT(audfilterParent), "use-buffering", FALSE, NULL);
                    }
                    g_free(audfilterParentName);
                }
            }
            g_free(elementName);
        }
