...
ifeq ($(PLATFORM_SOC), RIALTO)
SOURCES += rialto/rdk_gstreamer_utils_soc.cpp
PLATFORM_FLAGS += -DPLATFORM_AUDIODECODER='"$(RIALTO_AUDIODECODER)"'
PLATFORM_FLAGS += -DPLATFORM_BUFFERUNDERFLOW_SIGNAL='"$(RIALTO_BUFFERUNDERFLOW_SIGNAL)"'
PLATFORM_FLAGS += -DPLATFORM_VIRTUALDISPLAY_HEIGHT=$(RIALTO_VIRTUALDISPLAY_HEIGHT)
PLATFORM_FLAGS += -DPLATFORM_VIRTUALDISPLAY_WIDTH=$(RIALTO_VIRTUALDISPLAY_WIDTH)
endif
