...
The component follows a strict two-layer delegation pattern. The public-facing API layer provides thin wrapper functions that forward every call to a corresponding `_soc`-suffixed function defined in the vendor-supplied platform library. This separation ensures that the public API contract remains stable across platform integrations while all hardware-specific logic resides in the vendor layer. DRM functions further decouple from the vendor layer by resolving their implementation symbols at runtime using `dlsym`, keeping DRM shared object linkage entirely at runtime.

The northbound interface is a C++ header declaring all functions within the `rdk_gstreamer_utils` namespace. Streaming clients include this header and link against `librdkgstreamerutils.so`. The southbound interface is defined by a second header (`rdk_gstreamer_utils_soc.h`) listing the `_soc` function signatures that the vendor platform library must implement.

The library operates as a synchronous in-process utility library, invoked directly from the streaming client's threads.

Within a process lifetime, the component retains a small set of static function pointer caches used for `dlsym`-based DRM API resolution. All other state is transient, scoped to individual API calls.
...
- **Synchronization**: DRM API resolution relies on static local function pointers populated on first call. Callers are responsible for external synchronization if these functions are invoked concurrently from multiple threads.
...
| `rdk_gstreamer_utils` (Rialto platform variant) | Platform implementation for Rialto-enabled deployments. Uses Rialto GStreamer client sink elements as in-process IPC stubs to the Rialto server, which performs the actual media pipeline operations. Implements audio/video cap building, codec switching, audio gap handling, mixer integration, and low-latency audio player construction. | `rialto/rdk_gstreamer_utils_rltclient.cpp`                            |
...
| `retrieveGstElementByName`                      | Internal utility that recursively iterates a GStreamer bin hierarchy to locate an element whose name contains a given substring. Used by underflow callback installation in both builds, and by audio gap processing in the Rialto client variant.                                                                                                                                            | `rdk_gstreamer_utils.cpp`, `rialto/rdk_gstreamer_utils_rltclient.cpp` |
...
| `configAudioCap()`                                                  | Builds a GStreamer `GstCaps` object from `AudioAttributes` for AAC, EAC3, and LPCM codecs including SVP flag                                                      | `_soc`                       |
| `configVideoCap()`                                                  | Builds a GStreamer caps string for H.264, H.265/HEVC, and AV1 video streams including resolution, frame rate, and SVP flag                                        | `_soc`                       |
| `performAudioTrackCodecChannelSwitch()`                             | Manages mid-playback audio codec change: queries position, sends flush events, rebuilds caps, dispatches `switch-source` downstream event                         | `_soc`                       |
| `audioMixerConfigurePipeline()`                                     | Adds audio source and sink elements to a GStreamer pipeline, links them, and optionally attenuates output volume                                                  | `_soc`                       |
| `audioMixerGetDeviceInfo()`                                         | Returns preferred and maximum audio frame counts based on FIFO size constants                                                                                     | `_soc`                       |
| `constructLLAudioPlayer()`                                          | Builds a low-latency GStreamer audio pipeline, sets stream context, and configures low-latency and sync properties                                                | `_soc`                       |
...
- **Error Handling Strategy**: In the Rialto client implementation, missing GStreamer elements emit a GLib warning (`g_warning`) and return `false`. In the vendor-delegation build, `installUnderflowCallbackFromPlatform()` currently assumes the required elements exist. DRM resolution failures print to stdout and return an API-specific failure value (`-1`, `0`, or `false`). The underflow callback installation function returns `true` only when both audio and video signal handlers are connected with non-zero handler IDs.
...
| ------------------------------------------ | -------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUDIOMIXER_NOT_SUPPORTED`                 | preprocessor macro (reserved) | n/a               | Currently not referenced by this repository’s sources/Makefile. If audio-mixer compilation needs gating, define and consume a build flag (for example via `PLATFORM_FLAGS` with `-D...`) in the relevant source files. |
